...
def get_estados_prospeccion_api():
    """API para obtener lista de estados de prospección desde TblEstadoProspeccion"""
    try:
        from flask import current_app
        
        current_app.logger.info("=== OBTENER ESTADOS DE PROSPECCIÓN ===")
        ...
        try:
            ...
            current_app.logger.info("Ejecutando sp_ListarEstadosProspeccion()...")
            cursor.execute("CALL sp_ListarEstadosProspeccion()")
            
            estados = cursor.fetchall()
            
            current_app.logger.info(f"Se obtuvieron {len(estados)} estados de prospección")
            
            # Log detallado sin emojis
            for estado in estados:
                id_estado = estado.get('id_estado_prospeccion')
                nombre = estado.get('nombre')
                current_app.logger.info(f"  Estado: {nombre} (ID: {id_estado})")
            
            cursor.close()
            connection.close()
            
            current_app.logger.info("Retornando datos con éxito")
